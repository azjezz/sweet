...
async function invariant(): Awaitable<void> {
  ...
  // add non-shared service
  $container->add(Baz::class, new BazFactory(), false);
  // add shared service
  $container->add(Foo::class, new FooFactory(), true);
  // or simply via ->share()
  ...
  invariant($container->get(Foo::class) === $container->get(Foo::class), '7. Error');
  invariant($container->get(Bar::class) === $container->get(Bar::class), '8. Error');
  // baz is not shaerd
  invariant($container->get(Baz::class) !== $container->get(Baz::class), '9. Error');

  invariant($locator->get(Foo::class) === $container->get(Foo::class), '10. Error');
  invariant($locator->get(Bar::class) === $container->get(Bar::class), '11. Error');
  ...
  try {
    ...
    // Sweet :)
    invariant(
      'Service (Examples\Baz) not found: even though it exists in the inner service container.' ===
      $e->getMessage(),
      '13. Error'
    );
  }
